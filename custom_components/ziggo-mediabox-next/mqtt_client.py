...
class MqttClient():
    ...
    def __init__(self, clientId, username, password, setopid):
    ...
    def __on_connect(self, client, userdata, flags, resultCode):
        ...
        if (resultCode == 0):
            ...
            self._topic = userdata['householdId'] + "/" + self._setopid
            self._isConnected = True

            self._client.subscribe(userdata['householdId'] + "/" + self._clientId)
            _LOGGER.info("Subscribed to topic " + userdata['householdId'] + "/" + self._clientId)
            
            self._client.subscribe(self._topic)
            _LOGGER.info("Subscribed to topic " + self._topic) 
            
            self._client.subscribe(self._topic + "/status")
            _LOGGER.info("Subscribed to topic " + self._topic + "/status") 
    ...
    def makeIdString(self, stringLength=8):
